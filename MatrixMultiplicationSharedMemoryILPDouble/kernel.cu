...
__global__ void kernel_smem_3(double* a, double* b, int n, double* c)
{
...
// Ядро для GPU (матрица умножения)
__global__ void kernel_smem_4(double* a, double* b, int n, double* c)
{
    int bx = blockIdx.x, by = blockIdx.y;
    int tx = threadIdx.x, ty = threadIdx.y;
    int aBegin = n * BLOCK_SIZE * by, aEnd = aBegin + n - 1;
...
// Ядро для GPU (матрица умножения)
__global__ void kernel_smem_5(double* a, double* b, int n, double* c)
{
    int bx = blockIdx.x, by = blockIdx.y;
    int tx = threadIdx.x, ty = threadIdx.y;
    int aBegin = n * BLOCK_SIZE * by, aEnd = aBegin + n - 1;
